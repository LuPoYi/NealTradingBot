...
const gridTradingInput = async () => {
  ...
  console.log('網格單建立完成，尚未在bybit下單', gridTradingSettings)
  ...
  console.log(
    `將會執行[Market][Sell] ${qty} BTC * ${marketOrderCount} -> 目前價位為 ${latestPrice}，初始posistion為 -${
      qty * marketOrderCount
    }`
  )
  console.log('將會執行[Limit][Buy][Close] 價位分別為', limitClosePrice)
  console.log('將會執行[Limit][Sell][Open] 價位分別為', limitOpenPrice)
...
const gridTradingExecute = (uuid, latestPrice, priceList, side) => {
