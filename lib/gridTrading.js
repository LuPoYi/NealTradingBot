...
const { logger, loggerServer, loggerAPI, loggerWS } = require('./utils/logger')
  ...
  inquirer
    .prompt([
      {
        type: 'list',
        name: 'uuid',
        message: '請選擇要移除的網格，會將Bybit的掛單取消並不再追蹤；倉位要自己手動處理',
    ...
    .then(async (answers) => {
      ...
      if (gridTrading) {
        ...
        // Place Market Order to clear position
        // const marketOrder = {
        //   side: 'Buy',
        //   symbol: symbol,
        //   order_type: 'Market',
        ...
        // const marketOrderResult = await placeActiveOrder(marketOrder)
        // gridTradingObject.allOrderResults.push(marketOrderResult)
        // gridTradingObject.filledOrderSellIDs.push(marketOrderResult.order_id)
