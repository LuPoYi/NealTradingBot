      ...
      inquirer
        ...
        .then((answers) => {
          if (answers.isConfirm) {
            console.log('開始下單!')
            gridTradingExecute(uuid, startAt)
          } else {
            console.log('重新操作')
          }
        })
...
//
const gridTradingExecute = (uuid, startAt) => {
  redisClient.hget('gridTrading', uuid, (error, result) => {
    if (error) {
      console.log(error)
      throw error
    }
    gridTradingSettings = JSON.parse(result)
    console.log('gridTradingSettings', gridTradingSettings)
