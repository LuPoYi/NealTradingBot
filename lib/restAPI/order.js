...
const { apiLog } = require('../utils/helper')
...
const placeActiveOrder = ({
  ...
  apiLog('placeOrder', symbol, side, sideWithColor, `Price: ${price}, Qty: ${qty}`)
...
const cancelActiveOrder = ({ symbol: symbol, order_id: order_id }) => {
  let sideWithColor = side === 'Buy' ? bid('Buy') : ask('Sell')
  apiLog('cancelOrder', symbol, side, sideWithColor, `Price: ${price}, Qty: ${qty}`)
