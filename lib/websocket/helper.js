...
const { placeActiveOrder, getActiveOrder } = require('../restAPI')
...
const checkAndPlaceMissOrders = async ({ uuid }) => {
  ...
  let currentOrderBuyIDs = gridTradingObject.currentOrderBuyIDs
  let currentOrderSellIDs = gridTradingObject.currentOrderSellIDs

  const activeOrderResults = await getActiveOrder
  // get redis orderID
