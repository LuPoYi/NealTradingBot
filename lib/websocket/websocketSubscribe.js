...
const { wsLog, logColors, errorLog, systemLog, apiLog } = require('../utils/helper')
        ...
        for (const data of message?.data) {
          ...
          if (['Buy', 'Sell'].includes(side)) {
            ...
            for (const [key, result] of Object.entries(gridTradingResult)) {
              ...
              console.log('priceList', priceList, 'price', price, data)
            ...
            if (uuid) {
              ...
              errorLog('[Redis] isNotExist', order_id)
            }
          }
        }
        break
...
const placeOrderAndUpdateRedis = async ({ uuid, side, price }) => {
  ...
  apiLog(`placeOrder - Limit${side}`, `Price: ${primary(price)} Qty: ${qty}`)
